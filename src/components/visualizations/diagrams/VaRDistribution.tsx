...
export function VaRDistribution({ className }: VaRDistributionProps) {
  ...
  const tailAreaData =
    distribution
      ...
      .map((point, i) => `${i === 0 ? 'M' : 'L'} ${xScale(point.x)} ${yScale(point.y)}`)
      .join(' ') + ` L ${varX} ${yScale(0)} L ${xScale(-4)} ${yScale(0)} Z`;
    ...
    <div className={cn('w-full max-w-4xl mx-auto', className)}>
            ...
            {([95, 99, 99.9] as ConfidenceLevel[]).map((level) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  backgroundColor:
                    confidence === level ? 'rgba(99, 102, 241, 0.15)' : 'var(--color-surface-1)',
                ...
                style={{
                  ...
                  backgroundColor:
                    distributionType === type
                      ...
                      : 'var(--color-surface-1)',
                  color:
                    distributionType === type ? 'rgb(245, 158, 11)' : 'var(--color-text-secondary)',
      ...
      >
        ...
        <div style={{ fontSize: '13px', color: 'var(--color-text-muted)' }}>
          &quot;With {confidence}% confidence, worst daily loss won&apos;t exceed ${dollarVaR}
          M&quot;
        </div>
      </motion.div>
      ...
      {distributionType === 'fat-tails' && (
        ...
        >
          ...
          >
            Real market returns have &quot;fat tails&quot; - extreme events happen more often than a
            normal distribution predicts. The 2008 financial crisis saw moves that were supposed to
            be &quot;25 standard deviation events&quot; - essentially impossible under normal
            assumptions. VaR can severely underestimate tail risk.
          </div>
        </motion.div>
      )}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
          ...
          <div>
            ...
            <div
              style={{ fontSize: '13px', color: 'var(--color-text-secondary)', lineHeight: '1.6' }}
            >
              <strong>VaR tells you the threshold, not how bad it can get.</strong> During the 2008
              crisis, some banks lost 10x their VaR in a single day. A 99% VaR says nothing about
              what happens in that remaining 1% - it could be a small loss or a catastrophic
              wipeout. This is why regulators now require &quot;stressed VaR&quot; and
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        >
          ...
          <span
            style={{
              transform: showBetComparison ? 'rotate(180deg)' : 'rotate(0deg)',
        ...
        {showBetComparison && (
          ...
          >
              ...
              {betExamples.map((bet) => (
                <motion.div
                  ...
                  style={{
                    padding: '16px',
                    backgroundColor:
                      selectedBet === bet.id ? 'rgba(99, 102, 241, 0.1)' : 'var(--color-surface-1)',
                    borderRadius: '12px',
                    border:
                      selectedBet === bet.id
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    {bet.outcomes.map((outcome, i) => (
                      ...
                      >
                        ...
                        >
                          {outcome.probability * 100}%: {outcome.pnl >= 0 ? '+' : ''}
                          {outcome.pnl}
                        </span>
                      </div>
                    ))}
            ...
            >
              <strong style={{ color: 'var(--color-text-primary)' }}>Key Insight:</strong> Both bets
              have the same expected return (+$100), but Bet B has 8x higher VaR. Same reward, very
              different risk. This is why VaR matters for comparing investments with similar
              expected returns.
            </div>
          </motion.div>
        )}
