  ...
  return (
    <div className={cn('w-full', className)} style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        {scenarios.map((s, index) => (
          <button
            ...
            style={{
              padding: '12px 8px',
              borderRadius: '10px',
              border: scenarioIndex === index ? `2px solid ${s.statusColor}` : '2px solid var(--color-surface-2)',
              backgroundColor: scenarioIndex === index ? `${s.statusColor}15` : 'var(--color-surface-1)',
              cursor: 'pointer',
              transition: 'all 0.2s',
            }}
          >
            ...
            {scenarioIndex >= 2 && (
              ...
              >
                <div style={{
                  flex: 1,
                  height: '2px',
                  background: 'repeating-linear-gradient(90deg, rgb(239, 68, 68) 0px, rgb(239, 68, 68) 8px, transparent 8px, transparent 16px)',
                }} />
                <span style={{
                  fontSize: '11px',
                  fontWeight: 600,
              ...
              {scenario.equity >= 0 ? (
                <>
                  ...
                  >
                    ...
                    {equityHeight > 50 && (
                      <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.8)', marginTop: '2px' }}>
                        Equity
                      </span>
                    )}
                  </motion.div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '14px', color: 'var(--color-text-secondary)' }}>
            ...
            <span style={{ fontWeight: 700 }}>
              ${scenario.assets}B = ${scenario.liabilities}B + ${scenario.equity >= 0 ? '' : '('}${scenario.equity}B{scenario.equity < 0 && ')'}
            </span>
          </span>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          padding: '16px',
          backgroundColor: scenario.lossAmount > 0 ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)',
          borderRadius: '12px',
          textAlign: 'center',
        }}>
          <div style={{ fontSize: '11px', color: 'var(--color-text-muted)', marginBottom: '4px' }}>
          ...
          <div style={{ fontSize: '24px', fontWeight: 700, color: scenario.lossAmount > 0 ? 'rgb(239, 68, 68)' : 'rgb(16, 185, 129)' }}>
            ${scenario.lossAmount}B
          </div>
          <div style={{ fontSize: '10px', color: 'var(--color-text-muted)' }}>
            by equity
        ...
        <div style={{
          ...
          <div style={{ fontSize: '18px', fontWeight: 700, color: scenario.statusColor, textTransform: 'uppercase' }}>
            {scenario.status}
          </div>
          <div style={{ fontSize: '10px', color: 'var(--color-text-muted)' }}>
      ...
      >
        <div style={{ fontSize: '14px', fontWeight: 600, color: scenario.statusColor, marginBottom: '6px' }}>
          {scenario.name}
        </div>
        <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', lineHeight: '1.6' }}>
          {scenario.description}
          {scenario.status === 'insolvent' && (
            <span style={{ display: 'block', marginTop: '8px', color: 'rgb(239, 68, 68)', fontWeight: 500 }}>
            ...
            >
              <div style={{ padding: '0 20px 20px 20px' }}>
                <div style={{
                  ...
                  {[
                    { step: 1, name: 'Sell Assets', desc: 'Convert loans, securities to cash', color: 'rgb(59, 130, 246)' },
                    { step: 2, name: 'Pay Secured Creditors', desc: 'Creditors with collateral get paid first', color: 'rgb(99, 102, 241)' },
                    { step: 3, name: 'Pay Depositors', desc: 'FDIC insurance covers up to $250K', color: 'rgb(245, 158, 11)' },
                    { step: 4, name: 'Pay Unsecured Creditors', desc: 'Bondholders and other lenders', color: 'rgb(234, 88, 12)' },
                    { step: 5, name: 'Residual to Shareholders', desc: 'Equity holders get what remains (often $0)', color: 'rgb(16, 185, 129)' },
                  ].map((item) => (
                    <div
                      key={item.step}
                      style={{
                ...
                <div style={{
                  ...
                  <strong style={{ color: 'rgb(16, 185, 129)' }}>Key Insight:</strong>{' '}
                  Shareholders have a <em>residual claim</em> — they only get paid after all other obligations.
                  This is why equity is called the &quot;first-loss piece&quot; and why regulators require banks to maintain capital buffers.
                </div>
              </div>
            </motion.div>
          )}
      ...
      <div style={{
        ...
        <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', lineHeight: '1.7' }}>
          ...
          Regulators require banks to maintain minimum equity ratios (typically 8-10%) to ensure they can absorb
          losses without becoming insolvent. This protects depositors and prevents systemic crises. When equity
          falls below thresholds, regulators can force the bank to raise capital, restrict dividends, or even
          take control.
        </div>
      </div>
